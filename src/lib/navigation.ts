...
export function getMobileNavItems(navigation: NavigationItem[]): NavigationItem[] {
  const cacheKey = JSON.stringify(navigation.map(item => ({ label: item.label, children: item.children?.length })))
...
export function shouldShowMobileMenu(navigation: NavigationItem[]): boolean {
  ...
  return navigation.length > 3 || navigation.some(item => item.children && item.children.length > 0);
}
