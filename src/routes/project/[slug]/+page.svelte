...
<svelte:head>
	{#if post}
		...
		{#if post.images?.[0] && getImageAsset(post.images[0])}
			<meta
				property="og:image"
				content={urlFor(getImageAsset(post.images[0])).width(1200).height(630).url()}
			/>
		{/if}
	{/if}
</svelte:head>
...
{:else if post}
	...
	<nav
		class="post__nav absolute w-full text-white px-3 md:px-5 flex flex-row justify-between items-center pointer-events-none text-sm md:text-base"
	...
	>
		...
		{#if nextPost}
			<a
				href="/project/{nextPost.slug.current}"
				class="nav-link transition hover:text-red pointer-events-auto ml-auto"
			>
				next
			</a>
		{/if}
